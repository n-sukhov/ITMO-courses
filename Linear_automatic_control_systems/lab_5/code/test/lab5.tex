...
Передаточная функция: $W(s)=\frac{K(Ts+1)}{s}$, где $K=\frac{1}{-R_1 C}=,~~T=CR_2 =$


Типовое звено - \textit{изодромное.}
...
Весовая: $w(t)={\mathcal{L}}^{-1} \lbrace W(s)\rbrace =$


Переходная: $h(t)={\mathcal{L}}^{-1} \left\lbrace \frac{W(s)}{s}\right\rbrace =$


АЧХ: $A(\omega )=|W(j\omega )|=\sqrt{{\text{Re}}^2 (W)+{\text{Im}}^2 (W)}=$
