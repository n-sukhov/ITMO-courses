...
поэтому окружность $r=1$ притягивает соседние траектории. Для каждой точки
окружности собственные значения равны $-2$ и $0$. Нулевое собственное
значение соответствует движению вдоль множества равновесий, поэтому
отдельные точки окружности не являются асимптотически устойчивыми.
...
$(0,0)$. Её поведение необходимо исследовать нелинейным методом или по
фазовому портрету. Для точки $(1,1)$
\begin{equation}
  \begin{aligned}
    A_6(1,1)&=
    \begin{pmatrix}
...
$x_3=(x_1^3-x_1)/3$. Поэтому значения $x_1$ находятся из уравнения
\begin{equation}
...
второй точки собственные значения численно равны
\begin{equation}
  \lambda_1\approx1{,}3585,\qquad
  \lambda_{2,3}\approx-31{,}5312\pm0{,}4253i
\end{equation}
